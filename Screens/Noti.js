...
const Noti = () => {
    ...
    const Clearappointdata=async()=>{
       var x = firestore().collection('appointmentAnswer').where("userEmail", "==", email);
    ...
    useEffect(() => {
      firestore().collection("appointmentAnswer").where("userEmail", "==", email)
      .get()
    ...
    return (<View style={{justifyContent:'center',alignItems:'center',flex:1,backgroundColor:'white'}}>
        <Text style={{fontWeight:'bold',fontSize:30}}>{appointmentStat.Msg}</Text>
        <TouchableOpacity style={styles.appButtonContainer} onPress={() =>{console.log(appointmentStat.Msg); Clearappointdata()}}><Text style={styles.appButtonText}>Clear Appointment Status</Text></TouchableOpacity>
    </View>)}
